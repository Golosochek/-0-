...
void demonstrateIntersections() {
    ...
    cout << "Круги пересекаются: " << (doCirclesIntersect(c1, c2) ? "Да" : "Нет") << endl;
    cout << "Квадраты пересекаются: " << (doSquaresIntersect(s1, s2) ? "Да" : "Нет") << endl;
    cout << "Круг и квадрат пересекаются: " << (doCircleAndSquareIntersect(c1, s1) ? "Да" : "Нет") << endl << endl;
}
